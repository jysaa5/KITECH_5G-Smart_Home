...
<title>CCTV Log Detail View</title>
...
<!-- 메인 컨텐트 시작 -->
	<main role="main" class="container">
		<div class="d-flex align-items-center p-3 my-3 text-white-50 bg-purple rounded box-shadow">
			<div class="lh-100">
				<h6 class="mb-0 text-white lh-100">CCTV Log Detail View</h6>
				<small>Since 2020</small>
			</div>
		</div>

		<div class="my-3 p-3 bg-white rounded box-shadow">
			<h6 class="border-bottom border-gray pb-2 mb-0">CCTV Log Detail View</h6>
			<%-- ${listView} --%>
			<table class="table table-striped">
				    <tr>
					<th>no</th>
					<td>${cctvLogDetailView.cctvLogIdx}</td>
					</tr>
					<tr>
					<th>저장된 날짜</th>
					<td>${cctvLogDetailView.cctvLogDate}</td>
					</tr>
					<tr>
					...
					<%-- <td id="logdate" colspan="2">${cctvLogDetailView.cctvLogFile}</td> --%>
					<c:choose>
					<c:when test = "${cctvLogDetailView.cctvLogFile == ''}">
					존재하는 로그 데이터가 없습니다.
					</c:when>
					<c:when test = "${cctvLogDetailView.cctvLogFile != ''}">
					${cctvLogDetailView.cctvLogFile}
					</c:when>
					</c:choose>
					</td>
					</tr>
				</table>
				<div><a id="home_a" href="<c:url value="/cctv/cctvLogList"/>"><input type="button" class="btn btn-lg btn-block btn-outline-primary" value="뒤로가기" id="click-home"/></a></div>
